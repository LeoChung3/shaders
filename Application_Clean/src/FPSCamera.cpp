...
void FirstPersonCamera::update(float dt)
{
	...
	if (m_handler->keyHasBeenPressed()) {
		...
		if (m_handler->isKeyPressed(GLFW_KEY_UP)) { // lets me look up, down, left, right with the arrow keys
			arrowlook(0, 1);
		}
		...
		if (m_handler->isKeyPressed(GLFW_KEY_1)) // changes the true or false of the lights, letting them turn on and off
		{
			D_light = false;
		}
...
bool FirstPersonCamera::getD()
{
	return D_light;
}

bool FirstPersonCamera::getP()
{
	return P_light;
}

bool FirstPersonCamera::getS()
{
	return S_light;
}
...
void FirstPersonCamera::arrowlook(float x, float y)
{
	m_yaw += x;
	m_pitch += y;
